...
namespace BluePrinceArchipelago.Patches
{
    ...
    public static class FSMPatches
    {
        ...
        public static void RoomForcer(PlayMakerFSM fsm)
        {
            ...
            FsmTransition Slot1 = ForceDraft.AddTransition("1", "Draft Forced Slot 1");
            FsmTransition Slot3 = ForceDraft.AddTransition("3", "Draft Forced Slot 3");
            DraftForcedCheck.AddTransition("Continue Draft", "SLOT 2");
            DraftForcedCheck.AddTransition("Force Draft", "Force Room Draft");
            FsmInt Slot = fsm.GetIntVariable("SLOT");
            ...
            ForceDraft.AddLastAction(new IntCompare() { integer1 = Slot, integer2 = 2, lessThan = Slot1.FsmEvent, greaterThan = Slot3.FsmEvent, everyFrame = false });
            ForceDraft.AddLastAction(new SetGameObject() { everyFrame = false, gameObject = fsm.GetGameObjectVariable("RoomEngine"), variable = ForcedRoom2 });
            DraftForcedSlot1.AddLastAction(new SetGameObject() { everyFrame = false, gameObject = fsm.GetGameObjectVariable("RoomEngine"), variable = ForcedRoom });
            DraftForcedSlot3.AddLastAction(new SetGameObject() { everyFrame = false, gameObject = fsm.GetGameObjectVariable("RoomEngine"), variable = ForcedRoom3 });
            SendEvent PlanSelected = fsm.GetState("Slot 1").GetFirstActionOfType<SendEvent>();
            ForceDraft.AddLastAction(PlanSelected);
            DraftForcedSlot1.AddLastAction(PlanSelected);
            DraftForcedSlot3.AddLastAction(PlanSelected);
            ForceDraft.AddLastAction(PlanSelected);
            ForceDraft.RemoveTransitionsTo("FINISHED");
            DraftForcedSlot1.RemoveTransitionsTo("FINISHED");
            DraftForcedSlot3.RemoveTransitionsTo("FINISHED");
